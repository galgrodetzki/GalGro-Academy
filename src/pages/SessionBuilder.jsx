  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Click a drill to preview it — drag or press + to add it to your session"
      >
            ...
            <div className="mb-3">
              ...
              <div className="relative h-2 bg-bg-soft rounded-full overflow-hidden">
                <div
                  className={`absolute inset-y-0 left-0 transition-all ${
                    overTarget ? "bg-red-500" : progress >= 90 ? "bg-accent" : "bg-accent/70"
                  }`}
              ...
              <div className="flex items-center justify-between mt-1.5 text-xs">
                ...
                <span className="text-white/40">
                  {session.blocks.length} {session.blocks.length === 1 ? "drill" : "drills"}
                </span>
              </div>
            </div>
              ...
              {session.blocks.length === 0 ? (
                <div className="flex-1 flex items-center justify-center border-2 border-dashed border-bg-border rounded-xl p-8 text-center">
                  <div>
                    ...
                    <p className="text-xs text-white/30 mt-1">or click the + on any drill</p>
                  </div>
                </div>
              ) : (
                <SortableContext
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-display text-xl font-bold">Save Session</h2>
              <button
                onClick={() => setShowSaveModal(false)}
  ...
  return (
    <div ref={setNodeRef} style={style} className={`card ${isDragging ? "opacity-40 z-50" : ""}`}>
      <div className="flex items-stretch">
        ...
        <div className="flex-1 py-3 pr-3 min-w-0 pl-3">
          <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-white/40 mb-1">
            <span className="bg-accent/20 text-accent rounded w-5 h-5 flex items-center justify-center text-[10px] shrink-0">
              {index + 1}
            </span>
            <span>{info?.icon}</span>
            <span className="truncate">{info?.label}</span>
      ...
      >
        ...
        <div className="flex items-center gap-4 text-xs text-white/60 mb-6 pb-4 border-b border-bg-border">
          <span className="flex items-center gap-1.5">
            <Clock size={13} className="text-accent" /> {drill.dur} min
          </span>
          <span className="flex items-center gap-1.5">
            <Zap size={13} className="text-electric" /> {drill.reps}
          </span>
          {drill.eq?.length > 0 && (
            <span className="flex items-center gap-1.5">
              <Package size={13} /> {drill.eq.join(", ")}
            </span>
          )}
        </div>
